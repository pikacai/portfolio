...
function Ball({ size, color, flip, start, year }: BallProps) {
  ...
  return start ? (
    <div style={ballStyle} className="bg-white rounded-full absolute left-1/2 transform -translate-x-1/2">
      ...
      {!flip && <p style={yearTextPosition} className="text-3xl absolute top-1/2 -translate-y-1/2 text-right">{year}</p>}
      {flip && <p style={yearTextPosition} className="text-3xl absolute top-1/2 -translate-y-1/2">{year}</p>}
      {flip && (
...
function About_() {
  const timeline = [{
    year: 2018,
    html: <div className="w-full flex mb-[90px] ml-[90px] -gap-x-8">
  ...
  {
    year: 2019,
    html: <div className="w-full flex justify-end mb-[90px] pr-[90px] -gap-x-8">
      <div className="w-[200px] h-[300px] p-3 shadow-md rounded-sm bg-white flex flex-col">
        <img className="bg-gray-400 h-[80%] object-contain" src={notes}></img>
        <BoldText text="Passionate about _drawing_ since childhood" parentElement="p"></BoldText>
      </div>
    </div>
  }
        ...
        <div className="mt-[500px]">
          {
            timeline.map((entry, index) => (<div className="flex">
              ...
              <div className="w-[3px] min-h-[300px] shadow-md bg-blue-500 relative">
                ...
                <Ball size="28px" color="#22D1EE" flip={index % 2 !== 0} start={entry.year !== -1} year={entry.year} />
                {/* </span> */}
